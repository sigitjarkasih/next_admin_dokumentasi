...
import APIPenjualKomplainListById from "../../config/api/dokumentasi_penjual/komplainpesanantoko/listById";
import ImageArticlePenjualKomplainUploadTools from "../../components/molecules/vacationEdit/penjual/imageArtcilePenjualKomplainPesanan";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import Head from "next/head";
import APIPenjualKomplainUpdate from "../../config/api/dokumentasi_penjual/komplainpesanantoko/update";
import APIPenjualKomplainDelete from "../../config/api/dokumentasi_penjual/komplainpesanantoko/delete";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIPenjualKomplainListById({
    id: context.query.id,
  });
...
class PenjualKomplainPesananUpdate extends React.Component {
  state = {
  ...
  submitData = async (values) => {
    const resp = await APIPenjualKomplainUpdate(values);
    if (resp.data === "success") {
      this.setState({
        open: true,
        message: "Update Berhasil",
      });
      Router.push("/menu_penjual_komplainpesanan/");
    } else {
      this.setState({
        open: true,
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPenjualKomplainDelete(data);
    Router.push("/menu_penjual_komplainpesanan/");
  };
              ...
              <Widget.Paper>
                ...
                <Widget.Box>
                  <Box mb={3}>
                    ...
                    <ImageArticlePenjualKomplainUploadTools
                      id={this.state.id}
...
export default PenjualKomplainPesananUpdate;
