...
import APIPenjualKomplainCreate from "../../config/api/dokumentasi_penjual/komplainpesanantoko/create";

class PenjualKomplainPesananAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPenjualKomplainCreate(values);
    // console.log(resp);
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Dokumentasi Article Komplain Pesanan" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default PenjualKomplainPesananAdd;
