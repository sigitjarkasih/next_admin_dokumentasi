...
import APIPenjualProsesListById from "../../config/api/dokumentasi_penjual/prosespesanan/listById";
import ImageArticlePenjualProsesPesananUploadTools from "../../components/molecules/vacationEdit/penjual/imageArtcilePenjualProsesPesanan";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import Head from "next/head";
import APIPenjualProsesUpdate from "../../config/api/dokumentasi_penjual/prosespesanan/update";
import APIPenjualProsesDelete from "../../config/api/dokumentasi_penjual/prosespesanan/delete";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIPenjualProsesListById({
    id: context.query.id,
  });
...
class PenjualProsesPesananUpdate extends React.Component {
  state = {
  ...
  submitData = async (values) => {
    const resp = await APIPenjualProsesUpdate(values);
    if (resp.data === "success") {
      this.setState({
        open: true,
        message: "Update Berhasil",
      });
      Router.push("/menu_penjual_prosespesanan/");
    } else {
      this.setState({
        open: true,
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPenjualProsesDelete(data);
    Router.push("/menu_penjual_prosespesanan/");
  };
              ...
              <Widget.Paper>
                ...
                <Widget.Box>
                  <Box mb={3}>
                    ...
                    <ImageArticlePenjualProsesPesananUploadTools
                      id={this.state.id}
...
export default PenjualProsesPesananUpdate;
