...
import APIPenjualFiturCreate from "../../config/api/dokumentasi_penjual/fiturpenjual/create";

class PenjualFiturAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPenjualFiturCreate(values);
    // console.log(resp);
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Dokumentasi Article Fitur Penjual" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default PenjualFiturAdd;
