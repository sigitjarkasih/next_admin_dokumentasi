...
import APIPenjualFiturListById from "../../config/api/dokumentasi_penjual/fiturpenjual/listById";
import ImageArticlePenjualFiturUploadTools from "../../components/molecules/vacationEdit/penjual/imageArtcilePenjualFitur";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import Head from "next/head";
import APIPenjualFiturUpdate from "../../config/api/dokumentasi_penjual/fiturpenjual/update";
import APIPenjualFiturDelete from "../../config/api/dokumentasi_penjual/fiturpenjual/delete";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIPenjualFiturListById({
    id: context.query.id,
  });
...
class PenjualFiturUpdate extends React.Component {
  state = {
  ...
  submitData = async (values) => {
    const resp = await APIPenjualFiturUpdate(values);
    if (resp.data === "success") {
      this.setState({
        open: true,
        message: "Update Berhasil",
      });
      Router.push("/menu_penjual_fitur/");
    } else {
      this.setState({
        open: true,
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPenjualFiturDelete(data);
    Router.push("/menu_penjual_fitur/");
  };
              ...
              <Widget.Paper>
                ...
                <Widget.Box>
                  <Box mb={3}>
                    ...
                    <ImageArticlePenjualFiturUploadTools
                      id={this.state.id}
...
export default PenjualFiturUpdate;
