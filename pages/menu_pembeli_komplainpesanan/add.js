...
import APIPembeliKomplainCreate from "../../config/api/dokumentasi_pembeli/komplainpesanan/create";

class PembeliKomplainAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPembeliKomplainCreate(values);
    // console.log(resp);
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Create Article Komplain Pesanan" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default PembeliKomplainAdd;
