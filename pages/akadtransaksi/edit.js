...
import APIAkadTransaksiListById from "../../config/api/akadtransaksi/listById";
import ImageArticleAkadtransaksiUploadTools from "../../components/molecules/vacationEdit/imageArticleAkadtransaksiUploadTools";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import APIAkadTransaksiUpdate from "../../config/api/akadtransaksi/update";
import APIAkadTransaksiDelete from "../../config/api/akadtransaksi/delete";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIAkadTransaksiListById({
    id: context.query.id,
  });
...
class DonasiUpdate extends React.Component {
  ...
  submitData = async (values) => {
    const resp = await APIAkadTransaksiUpdate(values);
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIAkadTransaksiDelete(data);
    if (resp.data === "success") {
              ...
              <Widget.Paper>
                ...
                <Widget.Box>
                  <Box mb={3}>
                    ...
                    <ImageArticleAkadtransaksiUploadTools
                      id={this.state.id}
