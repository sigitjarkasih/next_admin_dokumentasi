...
import APIPembeliPembayaranListById from "../../config/api/dokumentasi_pembeli/pembayaran/listById";
import ImageArticlePembeliPembayaranUploadTools from "../../components/molecules/vacationEdit/pembeli/imageArticlePembeliPembayaran";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import Head from "next/head";
import APIPembeliPembayaranUpdate from "../../config/api/dokumentasi_pembeli/pembayaran/update";
import APIPembeliPembayaranDelete from "../../config/api/dokumentasi_pembeli/pembayaran/delete";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIPembeliPembayaranListById({
    id: context.query.id,
  });
...
class PembeliPembayaranUpdate extends React.Component {
  state = {
  ...
  submitData = async (values) => {
    const resp = await APIPembeliPembayaranUpdate(values);
    if (resp.data === "success") {
      this.setState({
        open: true,
        message: "Update Berhasil",
      });
      Router.push("/menu_pembeli_pembayaran/");
    } else {
      this.setState({
        open: true,
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPembeliPembayaranDelete(data);
    Router.push("/menu_pembeli_pembayaran/");
  };
            ...
            <Form>
              ...
              <Widget.Paper>
                ...
                <Stack direction="row">
                  ...
                  <Button href="/menu_pembeli_pembayaran/">
                    Daftar Artikel
                  </Button>
                </Stack>
                <Widget.Box>
                  <Box mb={3}>
                    ...
                    <ImageArticlePembeliPembayaranUploadTools
                      id={this.state.id}
...
export default PembeliPembayaranUpdate;
