...
import APIPembeliPembayaranCreate from "../../config/api/dokumentasi_pembeli/pembayaran/create";

class PembeliPembayaranAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPembeliPembayaranCreate(values);
    // console.log(resp);
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Dokumentasi Article Pembeli Pembayaran" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default PembeliPembayaranAdd;
