...
import APIEventCreate from "../../config/api/event/create"
...
class EventAdd extends React.Component {
  ...
  submitData = async (values) => {
    const resp = await APIEventCreate(values);
    if (resp.data != "") {
      this.context.setSnackbar({
        open: true,
        message: "Submit Berhasil",
      });
      Router.push(`/event/edit?id=${resp.data}`);
    } else {
      this.context.setSnackbar({
        open: true,
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Create Article Event" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
