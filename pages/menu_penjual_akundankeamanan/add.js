...
import APIPenjualAkunCreate from "../../config/api/dokumentasi_penjual/akundankeamanantoko/create";

class PenjualAkundanKeamananAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPenjualAkunCreate(values);
    // console.log(resp);
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Dokumentasi Article Penjual Akun dan Keamanan" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default PenjualAkundanKeamananAdd;
