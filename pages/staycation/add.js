...
import APIStaycationCreate from "../../config/api/staycation/create";

class StaycationAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIStaycationCreate(values);
    if (resp.data != "") {
      this.context.setSnackbar({
        open: true,
        message: "Submit Berhasil",
      });
      Router.push(`/staycation/edit?id=${resp.data}`);
    } else {
      this.context.setSnackbar({
        open: true,
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Create Article Staycation" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
