...
import APIStaycationListById from "../../config/api/staycation/listById";
import ImageArticleStaycationUploadTools from "../../components/molecules/vacationEdit/imageArticleStaycationUploadTools";
...
export async function getServerSideProps(context) {
  const res = await APIStaycationListById({
    id: context.query.id,
  });
...
class DonasiUpdate extends React.Component {
  ...
  submitData = async (values) => {
    const resp = await APIStaycationUpdate(values);
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIStaycationDelete(data);
    if (resp.data === "success") {
              ...
              <Widget.Paper>
                ...
                <Widget.Box>
                  <Box mb={3}>
                    ...
                    <ImageArticleStaycationUploadTools
                      id={this.state.id}
