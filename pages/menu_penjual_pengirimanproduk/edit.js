...
import APIPenjulaPengirimanListById from "../../config/api/dokumentasi_penjual/pengirimanproduk/listById";
import ImageArticlePenjualPengirimanProdukUploadTools from "../../components/molecules/vacationEdit/penjual/imageArtcilePenjualPengirimanProduk";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import Head from "next/head";
import APIPenjualPengirimanUpdate from "../../config/api/dokumentasi_penjual/pengirimanproduk/update";
import APIPenjualPengirimanDelete from "../../config/api/dokumentasi_penjual/pengirimanproduk/delete";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIPenjulaPengirimanListById({
    id: context.query.id,
  });
...
class PenjualPengirimanProdukUpdate extends React.Component {
  state = {
  ...
  submitData = async (values) => {
    const resp = await APIPenjualPengirimanUpdate(values);
    if (resp.data === "success") {
      this.setState({
        open: true,
        message: "Update Berhasil",
      });
      Router.push("/menu_penjual_pengirimanproduk/");
    } else {
      this.setState({
        open: true,
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPenjualPengirimanDelete(data);
    Router.push("/menu_penjual_pengirimanproduk/");
  };
            ...
            <Form>
              ...
              <Widget.Paper>
                ...
                <Stack direction="row">
                  ...
                  <Button href="/menu_penjual_fitur/">Daftar Artikel</Button>
                </Stack>
                <Widget.Box>
                  <Box mb={3}>
                    <Typography>Image</Typography>
                    <ImageArticlePenjualPengirimanProdukUploadTools
                      id={this.state.id}
...
export default PenjualPengirimanProdukUpdate;
