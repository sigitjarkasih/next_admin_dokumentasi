...
import APIPenjualLainnyaCreate from "../../config/api/dokumentasi_penjual/lainnyapenjual/create";

class PenjualLainnyaAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPenjualLainnyaCreate(values);
    // console.log(resp);
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Dokumentasi Article Penjual Lainnya" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default PenjualLainnyaAdd;
