...
import APIPembeliPengirimanCreate from "../../config/api/dokumentasi_pembeli/pengiriman/create";

class PembeliPengirimanAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPembeliPengirimanCreate(values);
    // console.log(resp);
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Create Article Pembeli Pengiriman" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default PembeliPengirimanAdd;
