...
import APIPembeliPengirimanListById from "../../config/api/dokumentasi_pembeli/pengiriman/listById";
import ImageArticlePembeliPengirimanUploadTools from "../../components/molecules/vacationEdit/pembeli/imageArticlePembeliPengiriman";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import Head from "next/head";
import APIPembeliPengirimanUpdate from "../../config/api/dokumentasi_pembeli/pengiriman/update";
import APIPembeliPengirimanDelete from "../../config/api/dokumentasi_pembeli/pengiriman/delete";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIPembeliPengirimanListById({
    id: context.query.id,
  });
...
class PembeliPengirimanUpdate extends React.Component {
  state = {
  ...
  submitData = async (values) => {
    const resp = await APIPembeliPengirimanUpdate(values);
    if (resp.data === "success") {
      this.setState({
        open: true,
        message: "Update Berhasil",
      });
      Router.push("/menu_pembeli_pengiriman/");
    } else {
      this.setState({
        open: true,
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPembeliPengirimanDelete(data);
    Router.push("/menu_pembeli_pengiriman/");
  };
            ...
            <Form>
              ...
              <Widget.Paper>
                ...
                <Stack direction="row">
                  ...
                  <Button href="/menu_pembeli_pengiriman/">
                    Daftar Artikel
                  </Button>
                </Stack>
                <Widget.Box>
                  <Box mb={3}>
                    ...
                    <ImageArticlePembeliPengirimanUploadTools
                      id={this.state.id}
...
export default PembeliPengirimanUpdate;
