...
import APIPenjualListById from "../../config/api/penjual/listById";
import ImageArticlePenjualUploadTools from "../../components/molecules/vacationEdit/imageArticlePenjualUploadTools";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import APIPenjualUpdate from "../../config/api/penjual/update";
import APIPenjualDelete from "../../config/api/penjual/delete";
...
export async function getServerSideProps(context) {
  const res = await APIPenjualListById({
    id: context.query.id,
  });
...
class PenjualUpdate extends React.Component {
  state = {
  ...
  submitData = async (values) => {
    const resp = await APIPenjualUpdate(values);
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPenjualDelete(data);
    if (resp.data === "success") {
            ...
            <Form>
              ...
              <Widget.Paper>
                ...
                <Stack direction="row">
                  ...
                  <Button href="/penjual/">Daftar Artikel</Button>
                </Stack>
                <Widget.Box>
                  <Box mb={3}>
                    <Typography>Image</Typography>
                    <ImageArticlePenjualUploadTools
                      id={this.state.id}
