...
import APIPenjualCreate from "../../config/api/penjual/create";

class PenjualAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPenjualCreate(values);
    if (resp.data != "") {
      this.context.setSnackbar({
        open: true,
        message: "Submit Berhasil",
      });
      Router.push(`/penjual/edit?id=${resp.data}`);
    } else {
      this.context.setSnackbar({
        open: true,
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Create Article Penjual" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
