...
import APIPenjualList from "../../config/api/penjual/list";
...
class ArticlePage extends React.Component {
  ...
  getData = async () => {
    const resp = await APIPenjualList({
    ...
    return (
      <Layout>
        <Widget.Paper elevation={3}>
          ...
          <Widget.PageHeader title="Dokumentasi Article Penjual" />
          <Link href="./penjual/add">
              ...
              <Widget.Table
                ...
                body={dataList.map(function (row) {
                  return [
                    ...
                    {
                      value: (
                        <Link href={`./penjual/edit?id=${row.id}`}>
