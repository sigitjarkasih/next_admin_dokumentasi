...
import APIPembeliPengembaliandanaListById from "../../config/api/dokumentasi_pembeli/pengembaliandana/listById";
import ImageArticlePembeliPengembalianDanaUploadTools from "../../components/molecules/vacationEdit/pembeli/imageArticlePembeliPengembalianDana";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import Head from "next/head";
import APIPembeliPengembaliandanaUpdate from "../../config/api/dokumentasi_pembeli/pengembaliandana/update";
import APIPembeliPengembaliandanaDelete from "../../config/api/dokumentasi_pembeli/pengembaliandana/delete";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIPembeliPengembaliandanaListById({
    id: context.query.id,
  });
...
class PengembalianDanaUpdate extends React.Component {
  state = {
  ...
  submitData = async (values) => {
    const resp = await APIPembeliPengembaliandanaUpdate(values);
    if (resp.data === "success") {
      this.setState({
        open: true,
        message: "Update Berhasil",
      });
      Router.push("/menu_pembeli_pengembaliandana/");
    } else {
      this.setState({
        open: true,
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPembeliPengembaliandanaDelete(data);
    Router.push("/menu_pembeli_pengembaliandana/");
  };
            ...
            <Form>
              ...
              <Widget.Paper>
                ...
                <Stack direction="row">
                  ...
                  <Button href="/menu_pembeli_pengembaliandana/">
                    Daftar Artikel
                  </Button>
                </Stack>
                <Widget.Box>
                  <Box mb={3}>
                    ...
                    <ImageArticlePembeliPengembalianDanaUploadTools
                      id={this.state.id}
...
export default PengembalianDanaUpdate;
