...
import APIPembeliPengembaliandanaCreate from "../../config/api/dokumentasi_pembeli/pengembaliandana/create";

class PengembalianDanaAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPembeliPengembaliandanaCreate(values);
    // console.log(resp);
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Create Article Pembeli Pengembalian Dana" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default PengembalianDanaAdd;
