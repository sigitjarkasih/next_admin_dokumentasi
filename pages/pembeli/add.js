...
import APIPembeliCreate from "../../config/api/pembeli/create";

class PembeliAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPembeliCreate(values);
    if (resp.data != "") {
      this.context.setSnackbar({
        open: true,
        message: "Submit Berhasil",
      });
      Router.push(`/pembeli/edit?id=${resp.data}`);
    } else {
      this.context.setSnackbar({
        open: true,
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Create Article Pembeli" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
