...
import APIPembeliList from "../../config/api/pembeli/list";
...
class ArticlePage extends React.Component {
  ...
  getData = async () => {
    const resp = await APIPembeliList({
    ...
    return (
      <Layout>
        <Widget.Paper elevation={3}>
          ...
          <Widget.PageHeader title="Dokumentasi Article Pembeli" />
          <Link href="./pembeli/add">
              ...
              <Widget.Table
                ...
                body={dataList.map(function (row) {
                  return [
                    ...
                    {
                      value: (
                        <Link href={`./pembeli/edit?id=${row.id}`}>
