...
import APIPembeliListById from "../../config/api/pembeli/listById";
import ImageArticlePembeliUploadTools from "../../components/molecules/vacationEdit/imageArticlePembeliUploadTools";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import APIPembeliDelete from "../../config/api/pembeli/delete";
import APIPembeliUpdate from "../../config/api/pembeli/update";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIPembeliListById({
    id: context.query.id,
  });
...
class DonasiUpdate extends React.Component {
  ...
  submitData = async (values) => {
    const resp = await APIPembeliUpdate(values);
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPembeliDelete(data);
    if (resp.data === "success") {
            ...
            <Form>
              ...
              <Widget.Paper>
                ...
                <Stack direction="row">
                  ...
                  <Button href="/pembeli/">Daftar Artikel</Button>
                </Stack>
                <Widget.Box>
                  <Box mb={3}>
                    <Typography>Image</Typography>
                    <ImageArticlePembeliUploadTools
                      id={this.state.id}
