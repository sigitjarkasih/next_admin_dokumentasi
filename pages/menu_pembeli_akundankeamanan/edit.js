...
import APIPembeliAkunListById from "../../config/api/dokumentasi_pembeli/akundankemanan/listById";
import ImageArticlePembeliAkunUploadTools from "../../components/molecules/vacationEdit/pembeli/imageArticlePembeliAkun";
import ConfirmationDialog from "../../components/molecules/vacationEdit/confirmationDialog";
import Router, { withRouter } from "next/router";
import Head from "next/head";
import APIPembeliAkunUpdate from "../../config/api/dokumentasi_pembeli/akundankemanan/update";
import APIPembeliAkunDelete from "../../config/api/dokumentasi_pembeli/akundankemanan/delete";
import Textarea from "../../components/molecules/vacationEdit/Textarea";

export async function getServerSideProps(context) {
  const res = await APIPembeliAkunListById({
    id: context.query.id,
  });
...
class PembeliAkundanKeamananUpdate extends React.Component {
  state = {
  ...
  submitData = async (values) => {
    const resp = await APIPembeliAkunUpdate(values);
  ...
  deleteArticle = async () => {
    ...
    const resp = await APIPembeliAkunDelete(data);
    Router.push("/menu_pembeli_akundankeamanan/");
  };
            ...
            <Form>
              ...
              <Widget.Paper>
                ...
                <Stack direction="row">
                  ...
                  <Button href="/menu_pembeli_akundankeamanan/">
                    Daftar Artikel
                  </Button>
                </Stack>
                <Widget.Box>
                  <Box mb={3}>
                    ...
                    <ImageArticlePembeliAkunUploadTools
                      id={this.state.id}
...
export default PembeliAkundanKeamananUpdate;
