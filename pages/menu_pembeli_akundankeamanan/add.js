...
import APIPembeliAkunCreate from "../../config/api/dokumentasi_pembeli/akundankemanan/create";

class PembeliAkundanKeamananAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIPembeliAkunCreate(values);
    // console.log(resp);
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Create Article Akun dan Keamanan" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default PembeliAkundanKeamananAdd;
