...
import APIDomepetMadrasahCreate from "../../config/api/dompetmadrasah/create";

class DompetMadrasahAdd extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      master_judul: "",
  ...
  submitData = async (values) => {
    const resp = await APIDomepetMadrasahCreate(values);
    if (resp.data != "") {
      this.context.setSnackbar({
        open: true,
        message: "Submit Berhasil",
      });
      Router.push(`/dompetmadrasah/edit?id=${resp.data}`);
    } else {
      this.context.setSnackbar({
        open: true,
      ...
      <Layout>
        ...
        >
          {(formik) => (
            <Form>
              <Widget.Paper>
                <Widget.PageHeader title="Create Article Dompet Madrasah" />
                <Widget.Box>
                  <FrameField
                    title="Master Judul"
                    child={
...
export default DompetMadrasahAdd;
